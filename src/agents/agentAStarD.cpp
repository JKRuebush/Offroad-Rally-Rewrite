...
// StarD is directionally aware. It can eliminate roughly half the map looks
// at each point based on the direction of the previous point in the path.
REGISTER_AGENT(AStarD)(MapInterface* const api) {
    Point start = api->getStart();
    Point finish = api->getFinish();
    std::map<Point, infoTuple> pointInfo;
    pointInfo.emplace(start,
                      infoTuple(0, start.distanceTo(finish) * 2, Point(-1, -1), Direction::None));
